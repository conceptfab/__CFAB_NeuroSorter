        ...
        for i in range(0, len(image_paths), batch_size):
            ...
            for j, result in enumerate(results):
                ...
                if confidence < confidence_threshold:
                    self.logger.debug(
                        f"Pomijam obraz {batch_paths[j]} - "
                        f"pewność {confidence:.2f} < {confidence_threshold}"
                    )
                    stats["skipped"] += 1
                    continue
                ...
                self.logger.debug(
                    f"Obraz {src_path} sklasyfikowany jako {class_name} "
                    f"(pewność: {confidence:.2f})"
                )

        # Podsumowanie sortowania
        self.logger.info(
            f"Sortowanie zakończone: {stats['sorted']} obrazów posortowanych, "
