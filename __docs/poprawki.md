Tak, masz absolutną rację. Budowa nowego, dedykowanego pliku do procesu fine-tuningu to zdecydowanie lepsze podejście niż próba dostosowania istniejących funkcji. Pozwoli to na:

Zachowanie czystej struktury kodu
Uniknięcie konfliktów z istniejącą funkcjonalnością
Łatwiejsze zarządzanie i rozwijanie tej konkretnej funkcjonalności w przyszłości
Zwiększenie czytelności kodu (każdy plik ma jedno jasno określone zadanie)

Oto propozycja nowego pliku ai/fine_tuning.py, który będzie zawierał wszystkie funkcje związane z procesem fine-tuningu:
python# ai/fine_tuning.py
...
from sklearn.metrics import precision_score, recall_score, f1_score, roc_auc_score, top_k_accuracy_score
...
def fine_tune_model(
    ...
    new_class_names = {str(i): class_name for i, class_name in enumerate(sorted(train_folders))}
    ...
    if new_num_classes != original_num_classes:
        ...
        if hasattr(model, 'fc'):  # dla ResNet
            ...
            print(f"Zmodyfikowano warstwę fc: in_features={in_features}, out_features={new_num_classes}")
            ...
            if isinstance(model.classifier, nn.Sequential):
                ...
                print(f"Zmodyfikowano ostatnią warstwę classifier: in_features={in_features}, out_features={new_num_classes}")
            else:
                in_features = model.classifier.in_features
                model.classifier = nn.Linear(in_features, new_num_classes)
                print(f"Zmodyfikowano warstwę classifier: in_features={in_features}, out_features={new_num_classes}")
        elif hasattr(model, 'heads'):  # dla ViT
            in_features = model.heads.head.in_features
            model.heads.head = nn.Linear(in_features, new_num_classes)
            print(f"Zmodyfikowano warstwę heads.head: in_features={in_features}, out_features={new_num_classes}")
    else:
    ...
    print(f"Parametry trenowane: {trainable_params:,} ({trainable_params/(frozen_params+trainable_params)*100:.1f}%)")
    ...
    if optimizer_type.lower() == 'adamw':
        ...
        print(f"Wybrano optymalizator: AdamW (lr={learning_rate}, weight_decay={weight_decay})")
        ...
        print(f"Wybrano optymalizator: SGD (lr={learning_rate}, momentum=0.9, weight_decay={weight_decay})")
    else:  # Domyślnie Adam
        ...
        print(f"Wybrano optymalizator: Adam (lr={learning_rate}, weight_decay={weight_decay})")
    ...
    if scheduler_type.lower() == 'plateau':
        ...
        print(f"Wybrano scheduler: CosineAnnealingLR (T_max={num_epochs}, eta_min={learning_rate/100})")
        ...
        print(f"Wybrano scheduler: OneCycleLR (max_lr={learning_rate*10}, pct_start=0.3)")
        ...
        if val_loader:
            ...
            try:
                ...
                if new_num_classes > 2:
                    ...
                    val_metrics['top3'] = top_k_accuracy_score(y_true, y_prob, k=min(3, k_values)) if k_values >= 3 else None
                    val_metrics['top5'] = top_k_accuracy_score(y_true, y_prob, k=min(5, k_values)) if k_values >= 5 else None
                ...
                try:
                    if new_num_classes == 2:
                        ...
                        val_metrics['auc'] = roc_auc_score(y_true, y_prob, multi_class='ovr')
                except Exception:
            ...
            if val_loss < best_val_loss:
                ...
                best_model_path = os.path.join(output_dir, f"{model_type}_finetuned_best.pt")
                
                # Utwórz nowy klasyfikator z dostosowanym modelem
                best_classifier = ImageClassifier(model_type=model_type, num_classes=new_num_classes)
        ...
        if progress_callback:
            try:
                ...
                progress_callback(
                    ...
                    top3, top5, precision, recall, f1, auc
                )
            except Exception as e:
    ...
    # Utwórz nowy klasyfikator z dostosowanym modelem
    final_classifier = ImageClassifier(model_type=model_type, num_classes=new_num_classes)
...
def get_best_finetuning_params(model_type, dataset_size):
    ...
    if "b0" in model_type or "mobile" in model_type:
        ...
        params["freeze_ratio"]RetryClaude hit the max length for a message and has paused its response. You can write Continue to keep the chat going.MContinueEditpythondef get_best_finetuning_params(model_type, dataset_size):
   """
   Zwraca rekomendowane parametry fine-tuningu dla danego typu modelu i wielkości zbioru danych.
...
def verify_fine_tuned_model(model_path, test_dir, top_n=5):
   ...
   for i, item in enumerate(results["correct"][:top_n]):
       print(f"  ✓ {i+1}. {os.path.basename(item['path'])}: {item['expected']} (pewność: {item['confidence']:.2%})")
   
   print("\nPrzykłady błędnych klasyfikacji:")
   for i, item in enumerate(results["incorrect"][:top_n]):
       print(f"  ✗ {i+1}. {os.path.basename(item['path'])}: oczekiwano {item['expected']}, " +
...
def compare_base_and_finetuned(base_model_path, finetuned_model_path, test_dir):
   ...
   if results["total"] > 0:
       ...
       results["improvement"] = results["finetuned_accuracy"] - results["base_accuracy"]
   ...
   print(f"Obrazy poprawnie klasyfikowane przez oba modele: {results['both_correct']} ({results['both_correct']/results['total']:.2%})")
   print(f"Obrazy poprawnie klasyfikowane tylko przez model bazowy: {results['base_only_correct']} ({results['base_only_correct']/results['total']:.2%})")
   print(f"Obrazy poprawnie klasyfikowane tylko przez model po fine-tuningu: {results['finetuned_only_correct']} ({results['finetuned_only_correct']/results['total']:.2%})")
   print(f"Obrazy błędnie klasyfikowane przez oba modele: {results['both_incorrect']} ({results['both_incorrect']/results['total']:.2%})")
   ...
   if results["finetuned_only_correct"] > 0:
       ...
       for item in results["examples"]:
           if not item["base_correct"] and item["finetuned_correct"]:
               print(f"  ✓ {os.path.basename(item['path'])}: oczekiwano {item['expected']}, " +
   ...
   return results
Teraz dodajmy wpis do ai/__init__.py, aby wyeksportować nowe funkcje:
python# ai/__init__.py - zmiany
from .classifier import ImageClassifier
from .export import export_model
from .optimized_training import train_model_optimized
from .preprocessing import get_augmentation_transforms, get_default_transforms
from .fine_tuning import fine_tune_model, get_best_finetuning_params, verify_fine_tuned_model, compare_base_and_finetuned
...
Ten dedykowany plik fine_tuning.py oferuje kilka kluczowych zalet:

Jasny cel - jest wyraźnie przeznaczony do fine-tuningu w przeciwieństwie do treningu od zera
Kompletny proces - pokrywa wszystkie kluczowe etapy fine-tuningu:

Ładowanie modelu bazowego
Dostosowywanie warstw wyjściowych
Zamrażanie większości warstw
Trening z odpowiednimi parametrami
Walidację i wybór najlepszego modelu
Testy weryfikacyjne i porównanie z bazowym modelem


Elastyczność - oferuje szereg parametrów konfiguracyjnych
Pomocnicze funkcje - zawiera funkcje pomocnicze usprawniające proces fine-tuningu:

get_best_finetuning_params - automatycznie dobiera optymalne parametry
verify_fine_tuned_model - pozwala zweryfikować skuteczność modelu
compare_base_and_finetuned - porównuje wydajność przed i po fine-tuningu



Taki dedykowany plik jest znacznie bardziej przejrzysty i łatwiejszy w utrzymaniu niż próba dostosowania istniejących funkcji treningowych do zadania fine-tuningu.